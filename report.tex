...
\begin{solution}
    Add entries {\tt alarmHandler} (address of handler), {\tt alarmInterval} (alarm interval), {\tt alarmTimeLeft} (number ticks left for next alarm) to {\tt struct proc}. Initialise them to {\tt 0}, {\tt -1} and {\tt -1}. This would ensure that the process initially has no alarm (or very long alarm interval).

    In {\tt sigalarm} syscall, set {\tt alarmHandler} and {\tt alarmInterval} to the arguments given. Also set {\tt alarmTimeLeft} to {\tt alarmInterval} as the countdown has started.
